...
func TestDeleted(t *testing.T) {
	tests := []struct {
		...
		{"Test1: ", GetLinkedList([]int{22, 2, 77, 6, 22, 76, 89}), 6, false, 22, 89, GetLinkedList([]int{22, 2, 77, 22, 76, 89})},
		{"Test1: ", GetLinkedList([]int{22, 2, 77, 6, 22, 76, 89}), 6, true, 22, 89, GetLinkedList([]int{22, 2, 77, 22, 76, 89})},
	}

	for _, tempTest := range tests {
		tempTest.input.Delete(tempTest.value, false)
		if !EqualLists(tempTest.input, tempTest.want) &&
			tempTest.head != tempTest.want.head.value &&
			tempTest.tail != tempTest.want.tail.value {
